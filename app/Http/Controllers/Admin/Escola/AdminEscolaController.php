...
class AdminEscolaController extends Controller
{
    ...
    public function show($id)
    {
        try {
            ...
            $projetos = Projeto::where('escola_id', '=', $escola->id)
                ...
                ->with('escola')
                ->get();
            return view('admin.escola.show', compact('escola', 'alunos', 'professores', 'projetos', 'qntProjetos'));
        } catch (\Exception $e) {
    ...
    public
    function update(EscolaUpdateFormRequest $request, $id)
    {
        try {
            $dataForm = $request->all() + ['tipoUser' => 'escola'];
