<table class="centered responsive-table highlight bordered" id="tabelaAvaliadores">
    ...
    <tbody>
        ...
        @if ($projeto->pivot->tipo == 'orientador')
            <tr>
                <td>{{$professor->id}}</td>
                ...
                @if ($professor ->nCertificadosGeradosOrientador == 0)
                <a class="tooltipped" target="_blank" data-position="top" data-delay="50" data-tooltip="Certificado"
                href=@if(Auth::user()->tipoUser == 'escola') "{{ route('certificado.professor.orientador', $professor->id) }}">
                 @elseif(Auth::user()->tipoUser == 'admin') "{{ route('certificado.professor.orientador', $professor->id) }}">@endif
